# Phase 8: Integration Report — GPT Narrative Engine

## Goal
Build the GPT analysis engine that takes the detection engine outputs (profile, warnings, locks, gap, belief-behavior gaps) and produces a cohesive narrative report. This follows the same 3-tier fallback pattern used by Tool2GPTAnalysis and GroundingGPT.

## Prerequisites
- Phases 1-3 complete (all detection engines working)
- OpenAI API key configured in Script Properties (`OPENAI_API_KEY`)

## Files to Read First
- `core/grounding/GroundingGPT.js` — lines 660-700 for `callGPT()` method
- `tools/tool2/Tool2GPTAnalysis.js` — lines 1-100 for 3-tier fallback pattern
- `tools/tool2/Tool2Fallbacks.js` — for hardcoded fallback pattern
- `core/CollectiveResults.js` — the detection engine outputs from Phases 1-3

## Architecture

```
Detection Engines (Phases 1-3)
    |
    v
IntegrationGPT.generateNarrative(summary)
    |
    ├── Tier 1: GPT-4o call with full context
    ├── Tier 2: Retry after 2 seconds
    └── Tier 3: Template-based fallback (no GPT)
    |
    v
...
### New File: core/IntegrationGPT.js
  ...
  buildUserPrompt(analysisData) {
    ...
    if (bbGaps && bbGaps.length > 0) {
      ...
      for (var g = 0; g < Math.min(bbGaps.length, 5); g++) {
        var gap2 = bbGaps[g];
        prompt += '"' + gap2.label + '" (' + gap2.tool + '): Belief=' + gap2.beliefScore + ', Behavior=' + gap2.behaviorScore + ', Gap=' + gap2.gap + ' — ' + gap2.direction + '\n';
      }
      prompt += '\n';
    }
...
## Why a Separate callGPT?

GroundingGPT has its own `callGPT`. Tool2GPTAnalysis has its own `callGPT`. Each GPT module is self-contained. This follows the established pattern — each module can be tested independently and does not depend on another module's GPT infrastructure.

If you want to centralize this later (a shared GPTService), that is a refactor for a future session. For now, match the existing pattern.

## Test Function

```javascript
function testPhase8_GPTNarrative() {
  var testClientId = 'TEST_STUDENT_EMAIL';

  // Run detection engines
  var summary = CollectiveResults.getStudentSummary(testClientId);
  var analysisData = {
    profile: CollectiveResults._detectProfile(summary),
    warnings: CollectiveResults._generateWarnings(summary),
    awarenessGap: CollectiveResults._calculateAwarenessGap(summary),
    locks: CollectiveResults._detectBeliefLocks(summary),
    bbGaps: CollectiveResults._detectBeliefBehaviorGaps(summary),
    summary: summary
  };

  Logger.log('=== INPUT DATA ===');
  Logger.log('Profile: ' + (analysisData.profile ? analysisData.profile.name : 'none'));
  Logger.log('Warnings: ' + analysisData.warnings.length);
  Logger.log('Locks: ' + analysisData.locks.length);
  Logger.log('Gap: ' + (analysisData.awarenessGap ? analysisData.awarenessGap.severity : 'none'));

  // Generate narrative
  var narrative = IntegrationGPT.generateNarrative(analysisData);

  Logger.log('=== NARRATIVE OUTPUT ===');
  Logger.log('Source: ' + narrative.source);
  Logger.log('Profile: ' + (narrative.profileNarrative ? narrative.profileNarrative.substring(0, 100) + '...' : 'empty'));
  Logger.log('Warnings: ' + (narrative.warningNarrative ? narrative.warningNarrative.substring(0, 100) + '...' : 'empty'));
  Logger.log('Locks: ' + (narrative.lockNarrative ? narrative.lockNarrative.substring(0, 100) + '...' : 'empty'));
  Logger.log('Gap: ' + (narrative.gapNarrative ? narrative.gapNarrative.substring(0, 100) + '...' : 'empty'));
  Logger.log('Synthesis: ' + (narrative.overallSynthesis ? narrative.overallSynthesis.substring(0, 100) + '...' : 'empty'));
  Logger.log('Action Items: ' + narrative.actionItems.length);
  for (var i = 0; i < narrative.actionItems.length; i++) {
    Logger.log('  ' + (i + 1) + '. ' + narrative.actionItems[i]);
  }
}
```
...
To test Tier 3 independently (without needing GPT to fail):

```javascript
function testPhase8_FallbackOnly() {
  var testClientId = 'TEST_STUDENT_EMAIL';
  ...
  var fallback = IntegrationGPT.generateFallbackNarrative(analysisData);

  Logger.log('=== FALLBACK NARRATIVE ===');
  Logger.log('Profile: ' + fallback.profileNarrative);
  Logger.log('---');
  Logger.log('Warnings: ' + fallback.warningNarrative);
  Logger.log('---');
  Logger.log('Locks: ' + fallback.lockNarrative);
  Logger.log('---');
  Logger.log('Gap: ' + fallback.gapNarrative);
  Logger.log('---');
  Logger.log('Synthesis: ' + fallback.overallSynthesis);
  Logger.log('---');
  Logger.log('Actions: ' + fallback.actionItems.join(' | '));
}
```

## Verification Checklist

- [ ] IntegrationGPT.js created in `core/` directory
- [ ] `generateNarrative()` returns narrative object with all 6 sections + source
- [ ] `buildSystemPrompt()` instructs direct tone, no contractions, structured output
- [ ] `buildUserPrompt()` includes all detection engine outputs
- [ ] `parseNarrativeResponse()` correctly extracts all 6 sections from GPT output
- [ ] `isValidNarrative()` requires overallSynthesis (50+ chars) and 2+ action items
- [ ] `generateFallbackNarrative()` produces readable output without any GPT call
- [ ] `callGPT()` matches the GroundingGPT pattern exactly
- [ ] Tier 1 → Tier 2 → Tier 3 fallback chain works correctly
- [ ] Test function runs successfully with GPT (check API key is set)
- [ ] Fallback test runs without API key
- [ ] No escaped apostrophes in any strings
- [ ] System prompt explicitly prohibits contractions
- [ ] All user-facing fallback text uses full words ("do not", "you are", etc.)
- [ ] GPT call stays under 30-second GAS limit (single call, ~4-6 seconds)
