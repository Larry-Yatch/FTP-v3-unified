/**
 * Tool3Tests.js
 * Google Apps Script test functions for Tool 3
 *
 ...
 */
function test_Tool3_FormRendering() {
  const testClient = 'TEST_CLIENT_001';
  const results = {
    test: 'Form Rendering',
    passed: [],
    failed: [],
  ...
  try {
    ...
    for (let page = 2; page <= 7; page++) {
      try {
        const pageHtml = Tool3.render(testClient, page);
 ...
 */
function test_Tool3_ScoringCalculations() {
  const results = {
  ...
  try {
    ...
    try {
      ...
      try {
        GroundingScoring.calculateScores(badResponses, Tool3.config.subdomains);
 ...
 */
function test_Tool3_FallbackSystem() {
  const results = {
  ...
  try {
    ...
    Tool3.config.subdomains.forEach(subdomain => {
      try {
        ...
        // Test moderate level (average -0.5)
        const moderateScores = {
          belief: -1,
          behavior: -1,
          feeling: 0, // This will fail, but testing logic
          consequence: 0
        };
    ...
    try {
      const domain1Fallback = GroundingFallbacks.getDomainFallback(
        'tool3',
        { key: 'domain1', name: 'False Self-View', description: 'Test' },
        {},
        80
      );
    ...
    try {
      const domain2Fallback = GroundingFallbacks.getDomainFallback(
        'tool3',
        { key: 'domain2', name: 'External Validation', description: 'Test' },
        {},
        70
      );
 ...
 */
function test_Tool3_DataFlowIntegration() {
  const testClient = 'TEST_CLIENT_DATA_FLOW';
  const results = {
  ...
  try {
    ...
    Tool3.config.subdomains.forEach(subdomain => {
      ...
      formData[`${subdomain.key}_open_response`] = `Test response for ${subdomain.label}. This is a longer response to ensure we have enough content for GPT analysis. I'm describing my experience with this pattern and how it shows up in my financial life.`;
    });
    ...
    try {
      const responses = Tool3.extractResponses(formData);
    ...
    try {
      ...
      const reportHtml = Tool3.generateReport(testClient, scoringResult, mockGPTInsights);
      ...
      if (!html.includes('Identity & Validation')) {
    ...
    try {
      if (CONFIG.TOOLS.TOOL3.ID !== 'tool3') {
        throw new Error('Config tool ID incorrect');
      }
      if (CONFIG.TOOLS.TOOL3.PAGES !== 7) {
        throw new Error('Config pages incorrect');
      }
      if (CONFIG.TOOLS.TOOL3.QUESTIONS !== 30) {
 ...
 */
function test_Tool3_ContentAccuracy() {
  const results = {
  ...
  try {
    ...
    try {
      if (Tool3.config.subdomains.length !== 6) {
        throw new Error(`Expected 6 subdomains, found ${Tool3.config.subdomains.length}`);
      }
    ...
    Tool3.config.subdomains.forEach((subdomain, idx) => {
      try {
    ...
    // Test domain names
    try {
      if (Tool3.config.domain1Name !== 'False Self-View') {
        throw new Error('Domain 1 name incorrect');
      }
      if (Tool3.config.domain2Name !== 'External Validation') {
    ...
    // Test tool metadata
    try {
      if (Tool3.config.id !== 'tool3') throw new Error('Tool ID incorrect');
      if (!Tool3.config.name.includes('Identity')) throw new Error('Tool name incorrect');
      if (!Tool3.config.scoreName) throw new Error('Score name missing');
 ...
 */
function runAllTool3Tests() {
  Logger.log('\n\n🧪 ========================================');
  Logger.log('    TOOL 3 COMPREHENSIVE TEST SUITE');
  Logger.log('========================================\n');

  const allResults = [];

  // Run each test
  allResults.push(test_Tool3_FormRendering());
  allResults.push(test_Tool3_ScoringCalculations());
  allResults.push(test_Tool3_FallbackSystem());
  allResults.push(test_Tool3_DataFlowIntegration());
  allResults.push(test_Tool3_ContentAccuracy());
