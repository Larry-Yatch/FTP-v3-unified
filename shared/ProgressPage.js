...
const ProgressPage = {
  ...
  _getScripts(clientId, isCoach) {
    return 'var _clientId = "' + clientId + '";'
      ...
      // After cross-tool finishes, chain-load all visible tool narratives sequentially
      + 'var _narrativeQueue = [];'
      + 'var _narrativeLoading = false;'
      + 'function loadAllVisibleToolNarratives() {'
      + '  var tools = ["tool1","tool2","tool3","tool5","tool7"];'
      + '  for (var i = 0; i < tools.length; i++) {'
      + '    var container = document.getElementById("narrative-" + tools[i]);'
      + '    if (container && container.dataset.loaded !== "true") {'
      + '      _narrativeQueue.push(tools[i]);'
      + '    }'
      + '  }'
      + '  processNarrativeQueue();'
      + '}'
      + 'function processNarrativeQueue() {'
      + '  if (_narrativeLoading || _narrativeQueue.length === 0) return;'
      + '  _narrativeLoading = true;'
      + '  var toolId = _narrativeQueue.shift();'
      + '  var container = document.getElementById("narrative-" + toolId);'
      + '  if (!container || container.dataset.loaded === "true") { _narrativeLoading = false; processNarrativeQueue(); return; }'
      + '  container.dataset.loaded = "true";'
      + '  google.script.run'
      + '    .withSuccessHandler(function(result) { renderToolNarrative("narrative-" + toolId, result); _narrativeLoading = false; processNarrativeQueue(); })'
      + '    .withFailureHandler(function(err) { renderNarrativeError("narrative-" + toolId); _narrativeLoading = false; processNarrativeQueue(); })'
      ...
      + '  _narrativeQueue.unshift(toolId);'
      + '  processNarrativeQueue();'
      + '}'
